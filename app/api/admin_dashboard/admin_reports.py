...
admin_reports_bp = Blueprint(
    "admin_reports_bp", __name__, url_prefix="/api/admin/reports"
)
...
def get_demographics():
    ...
    age_groups = [
        {
            "age": "18–24",
            "count": db.session.query(func.count(Customers.id))
            .filter(Customers.age.between(18, 24))
            .scalar()
            or 0,
        },
        {
            "age": "25–34",
            "count": db.session.query(func.count(Customers.id))
            .filter(Customers.age.between(25, 34))
            .scalar()
            or 0,
        },
        {
            "age": "35–44",
            "count": db.session.query(func.count(Customers.id))
            .filter(Customers.age.between(35, 44))
            .scalar()
            or 0,
        },
        {
...
def get_revenue():
    ...
    salon_data = [
        {"name": name, "total": float(total)} for name, total in top_salon_revenue
    ]
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        if selected.get("engagement"):
            engagement_data = db.session.query(
                Appointment.id, Appointment.status, Appointment.created_at
            ).all()
            df_engagement = pd.DataFrame(
                engagement_data, columns=["Appointment ID", "Status", "Created At"]
            )
            df_engagement.to_excel(writer, sheet_name="Engagement", index=False)

        if selected.get("revenue"):
            invoice_data = db.session.query(
                Invoice.id, Invoice.total, Invoice.created_at
            ).all()
            df_revenue = pd.DataFrame(
                invoice_data, columns=["Invoice ID", "Total", "Created At"]
            )
