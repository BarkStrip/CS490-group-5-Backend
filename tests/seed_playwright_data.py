    ...
    with app.app_context():
        ...
        if not customer_profile:
            customer_profile = Customers(
                ...
                address="12 Main Ave",
                gender="Other",  # filled in reasonably
        ...
        if not owner_profile:
            owner_profile = SalonOwners(
                ...
                address="Owner Address",  # filled in
            )
        ...
        if not salon:
            salon = Salon(
                ...
                name="PlaywrightTest",
                # Newark-ish coordinates; required non-null
                latitude=40.735657,
                longitude=-74.172363,
        ...
        # ------------------------------------------------------------------
        # 3a) SALON HOURS 7 days/week, 09:00–17:00
        #     weekday: 0..6
        # ------------------------------------------------------------------
        for weekday in range(7):
            hours = SalonHours.query.filter_by(
        ...
        if not verify:
            verify = SalonVerify(
                salon_id=salon.id,
                status="APPROVED",
                admin_id=None,  # no specific admin; okay, column is nullable
            )
            db.session.add(verify)
            print("  ✅ Created SalonVerify with status=APPROVED")
        else:
            # if it exists but not approved, we could bump it to APPROVED
        ...
        if not lp:
            lp = LoyaltyProgram(
                ...
                reward_type=None,
                # other fields (points_per_dollar, reward_description,
                # points_for_reward) will use DB defaults
            )
            db.session.add(lp)
        ...
        # ------------------------------------------------------------------
        emp_user = AuthUser.query.filter_by(
        ...
        if not employee_profile:
            employee_profile = Employees(
                ...
                first_name="Playwright",
                last_name="TesterEmp",
                phone_number="0000000000",  # filled in, since not specified
